...
const BRAWSER = { name: "Chrome", version: "83.0.4103.116", major: "83" };
...
test({
  ...
  fn() {
    ...
    const brawser = uaParser.getBrowser();

    assertEquals(typeof brawser, "object");
    assertEquals(brawser, BRAWSER);
  },
});
...
test({
  ...
  fn() {
    ...
    assertEquals(result.browser, BRAWSER);
